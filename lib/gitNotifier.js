 ...
 */
GitNotifier.prototype.cloneRepoIfNotExists = async function(repoUrl) {
    const localRepoPath = this.localPath(repoUrl);

    const exists = await Bluebird.fromCallback(cb => fs.exists(localRepoPath, cb));
    if (!exists) {
 ...
 */
GitNotifier.prototype.checkForNewCommits = async function(repoUrl) {
    const localRepoPath = this.localPath(repoUrl);
 ...
 * @param {function} callback
 */
GitNotifier.prototype.sendEmailNotification = async function(repoUrl, ansiLogAndDiff, localSha1, remoteSha1) {
...
GitNotifier.prototype.buildHtmlDiff = function(repoUrl, ansiTxt, localSha1, remoteSha1) {
    ...
    const encodedTxt = entities.encode(ansiTxt);
