...
class GitNotifier {
    ...
    async checkForNewCommits(repoUrl) {
        ...
        if (localSha1 === remoteSha1) {
            ...
            const [ansiLog, ansiDiff] = await Promise.all([
                gitExec('log', { ...gitOpts, stat: true }, gitArgs),
    ...
    async sendEmailNotification(repoUrl, ansiLogAndDiff, localSha1, remoteSha1) {
        ...
        if (this.mailTransporter === undefined) {
            ...
            this.mailTransporter = nodemailer.createTransport(config.get('email.smtpOptions'));
        }
    ...
    buildHtmlDiff(repoUrl, ansiTxt, localSha1, remoteSha1) {
        ...
        if (config.has('ansiSizeLimitBytes')) {
            modifiedAnsiText = truncate(modifiedAnsiText, {
                length: config.get('ansiSizeLimitBytes'),
                omission: ' ... [truncated]',
            });
        }
        ...
        if (tabsToSpaces) {
            modifiedAnsiText = modifiedAnsiText.replace(/\t/g, ' '.repeat(tabsToSpaces));
        }
